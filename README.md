...
以 FASHN VTON v1.5 與繁體中文 Gradio 介面，在本機將人物照片與商品照片合成試穿圖，
再用筆刷選區修補手部、袖口、衣料或背景等局部。

生成成功後，圖片會自動載入「局部修補」，不必另按載入按鈕。
目前是實驗工具：已執行官方範例 GPU 生成與局部修補測試，但不保證手指、人物身分、
商品圖案或遮擋關係正確，也不能判定實際尺寸與合身程度。
實測結果及未完成項目見 [驗證紀錄](docs/verification.md)。
...
若要繼續人工修補：展開「生成設定」，取消勾選「自動保留原手」，再按「開始試穿」。
這會生成明確標示為待人工檢查的候選，不會放寬門檻，也不代表手部已修好。

## 局部修補

1. 直接使用自動載入的試穿結果，或在修補區上傳現有圖片。
...
修補不覆寫原始檔案；「將目前試穿結果載入修補區」可重新從試穿圖開始。
新的試穿成功會替換編輯器底圖並清除舊選區，請先下載要保留的修補成果。
試穿生成中或失敗時，現有修補底圖與選區不會被清空。
